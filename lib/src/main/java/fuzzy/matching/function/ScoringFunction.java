...
public interface ScoringFunction extends BiFunction<Match, List<Score>, Score> {
   ...
   * To get a balanced average for 2 Match object which do not have same number of
   * childScores.
   ...
   * to children.
   * It can be used for aggregating Elements to Documents, where weights can be
   * provided at Element level
   *
   ...
   * children match above a score of 0.9,
   ...
   * aggregating the child Element scores.
   * This combines the benefits of weights and exponential increase when
  ...
  static double getSumOfWeightedResult(List<Score> childScoreList) {
    return (childScoreList.stream()
        .mapToDouble(d -> d.getResult() * d.getMatch().getWeight())).sum();
  }
  ...
  /**
   * Get the child scores which are not perfect matches.
   *
   * @param childScoreList the list of child scores
   * @return the list of child scores which are not perfect matches
   */
  ...
  /**
   * Get the child scores which are perfect matches.
   *
   * @param childScoreList the list of child scores
   * @return the list of child scores which are perfect matches
   */
