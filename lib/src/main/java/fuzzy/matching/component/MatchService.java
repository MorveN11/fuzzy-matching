...
/**
 * Entry Point for Fuzzy Matching. This class provides different ways to accept
 * Documents for primarily 3 use case
 * <p>
 * 1. De-duplication of data - Where for a given list of documents it finds
 * duplicates
 * 2. Check duplicate for a new data - Where it checks for a new Document a
 * duplicate is present in existing list
 * 3. Check duplicates for bulk inserts - Similar to 2, where a list of new
 * Documents is checked against existing
 * </p>
 * This also has similar implementation to aggregate results in different
 * formats.
 */
public class MatchService {

  /**
   * Use this for De-duplication of data, where for a given list of documents it
   * finds duplicates
   * Data is aggregated by a given Document.
   *
   * @param documents the list of documents to match against
   * @return a map containing the grouping of each document and its corresponding
   *         matches
   */
  ...
  /**
   * Use this to check duplicates for bulk inserts, where a list of new Documents
   * is checked against existing list
   * Data is aggregated by a given Document.
   *
   ...
   *         matches
   */
  public Map<Document, List<Match<Document>>> applyMatch(
      List<Document> documents, List<Document> matchWith) {
    DocumentMatch documentMatch = new DocumentMatch();
  ...
  /**
   * Use this to check duplicate for a new record, where it checks whether a new
   * Document is a duplicate in existing list
   * Data is aggregated by a given Document.
   *
   ...
   */
  public Map<Document, List<Match<Document>>> applyMatch(
      Document document, List<Document> matchWith) {
   ...
   * @param document  the document to match
   * @param matchWith the list of documents to match against
   * @return a map containing the grouping of each document id and its
   *         corresponding matches
   */
  public Map<String, List<Match<Document>>> applyMatchByDocId(
      Document document, List<Document> matchWith) {
   ...
   *
   * @param documents the list of documents to match against
   * @return a set containing the grouping of all relevant matches. So if A
   *         matches B, and B matches C. They will be grouped together
   */
  ...
  private void groupSimilar(Map<String, List<Match<Document>>> matchMap, String key,
      Set<Match<Document>> matchGroups) {
