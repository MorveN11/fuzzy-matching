...
/**
 * <p>
 * A generic class to hold the match between 2 objects and the score of the
 * match result.
 * A match between similar Token, Element or Document is represented by this
 * class.
 * </p>
 * The "data" and "matchedWith" object holds the 2 records that matched. And
 * "score" represents the match for these 2 objects.
 * "childScore" is used by ScoringFunction to aggregate and calculate the
 * "score" value
 */
public class Match<T extends Matchable> {
  ...
  /**
   * Constructor to create a Match object with the given data and matchedWith.
   *
   * @param t           The data object
   * @param matchedWith The matched object
   * @param childScores The list of child scores to be used for scoring
   */
  ...
  private List<Score> getMaxDistinctScores(List<Score> scoreList) {
    ...
    return map.entrySet().stream().map(
        entry -> entry.getValue().get()).collect(Collectors.toList());
  }
